...
the model class to the rows and columns in the database.
	...
	use Phlite\Db;
    class User extends Db\ModelBase {
		static $meta = array(
			'table' => 'user',
			'pk' => ['id'],
		);
...
any time, an instance of a model is to be created and it is already in the 
...
how the instances were retrieved from the database nor what time it was 
retrieved.